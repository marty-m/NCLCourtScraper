...
func Court8() {
	...
	defer cancel()
	//Sequence to navigate to Court 8 page on booking site
	err := chromedp.Run(ctx,
		...
		chromedp.Click("input[value=\"Basketball Hoop Court 8\"", chromedp.NodeVisible),
		chromedp.WaitReady("head[id=ctl00_Head1]"),
	)
	...
	for i := 0; i < 3; i++ {
		...
		if jsres != 0 {
			...
			for i := 0; i < jsres; i++ {
				...
				fmt.Println(*NewBooking(8, courtDate))
				AddBooking(*NewBooking(8, courtDate))
